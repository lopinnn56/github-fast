...
// 逐项入场动画只对前几项生效，避免批量结果数百个 animationDelay 排队；
// 新一次转换时从头计数（与旧版 renderResults 内 animCounter=0 行为一致）
let animCounter = 0;
watch(links, function () { animCounter = 0; });
function nextDelay() {
    animCounter++;
...
<template>
    <div class="results" aria-live="polite" :hidden="!hasLinks">
        ...
        <div class="result-list">
            <div v-for="g in groups" :key="g.url" class="link-group">
